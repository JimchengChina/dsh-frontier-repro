...
const ARTIFACT_RULES = [
  ...
  ['model', url => url.hostname === 'huggingface.co' && !url.pathname.startsWith('/datasets/')],
...
function normalizeItem(source, item, now) {
  ...
  return {
    ...
    artifacts: classifyArtifacts(links, url),
...
export async function collectSource(source, options = {}) {
  if (source.type === 'arxiv') {
    ...
    return { items: parseArxiv(response.body, source), warnings: [] }
  }
  if (source.type === 'feed') {

...
async function enrichGitHubArtifacts(records, options) {
  ...
  const settled = await mapConcurrent(selected, options.pageConcurrency ?? 3, async (identity) => {
    ...
    try {
      const repository = await fetchJson(`https://api.github.com/repos/${path}`, { ...options, headers })
      const branch = encodeURIComponent(repository.default_branch ?? 'main')
      const commit = await fetchJson(`https://api.github.com/repos/${path}/commits/${branch}`, { ...options, headers })
      return { key: identity.rootUrl.toLowerCase(), patch: githubRepositoryEnrichment(repository, commit, identity) }
    } catch (error) {
